...
extension DataViewController {
    func makeFirestore() {
        ...
        let month = MonthStringToInt.init(rawValue: String(dateArr[0]))?.StringToInt()
        let day = DayStringToInt.init(rawValue: String(dateArr[1]))?.StringToInt()
        
        let date_path = "\(month!)_\(day!)"
            ...
            if let error = error {
                ...
                if let data = snapshot.data() {
                    ...
                    if let url = data["url"] as? String {
                        _self.liveView.load(WebLiveGet.loadVideo(url: url))
                    }
                    else {
                        //error driving
                    }
